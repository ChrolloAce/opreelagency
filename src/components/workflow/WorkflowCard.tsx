...
interface WorkflowCardProps {
  title: ReactNode; // Changed from string to ReactNode to support <br/>
  subtitle: string;
  gradient: string;
  image?: string;
  icon?: ReactNode;
  children?: ReactNode; // Explicitly optional
  className?: string;
}

export function WorkflowCard({ title, subtitle, gradient, image, icon, children, className = "" }: WorkflowCardProps) {
  return (
    <div 
    ...
    >
      ...
      <div className="relative z-10 mt-auto">
        ...
        <p className="text-sm leading-relaxed text-slate-200 mt-2 font-medium drop-shadow-sm">{subtitle}</p>
      </div>
